...
public class BoardBot : MonoBehaviour
{
    ...
    public class Position
    {
        ...
        {
            List<Move> moves = new List<Move>();
            if (rankInBounds(rank+1) && isAttackableNoVoid(piece, getPiece(rank+1, file+2)))
            {
            ...
            if (rankInBounds(rank+2) && isAttackableNoVoid(piece, getPiece(rank+2, file+1)))
            {
        ...
        public List<Move> movementBishop(byte rank, byte file, byte piece)
        {
            ...
            byte i = 1;
            while (rankInBounds(rank+i) && isAttackableNoVoid(piece, getPiece(rank+i, file+i)))
            {
                if (!isBlank(getPiece(rank+i, file+i)))
                {
            ...
            while (rankInBounds(rank-i) && isAttackableNoVoid(piece, getPiece(rank-i, file-i)))
            {
                if (isPiece(getPiece(rank-i, file-i)))
                {
                    if (isEnemy(piece, getPiece(rank-i, file-i)))
                    {
                        moves.Add(new Move{rankFrom = rank, fileFrom = file, rankTo = (byte)(rank-i), fileTo = fileOverload(file-i)});
        ...
        public List<Move> movementRook(byte rank, byte file, byte piece)
        {
            ...
            byte i = 1;
            while (rankInBounds(rank+i) && isAttackableNoVoid(piece, getPiece(rank+i, file)))
            {
                if (isPiece(getPiece(rank+i, file)))
                {
            ...
            i = 1;
            while (rankInBounds(rank) && isAttackableNoVoid(piece, getPiece(rank, file+i)))
            {
                if (isPiece(getPiece(rank, file+i)))
                {
        ...
        {
            List<Move> moves = new List<Move>();
            if (rankInBounds(rank+1) && isAttackableNoVoid(piece, getPiece(rank+1, file)))
            {
                moves.Add(new Move{rankFrom = rank, fileFrom = file, rankTo = (byte)(rank+1), fileTo = file});
            }
            if (rankInBounds(rank-1) && isAttackableNoVoid(piece, getPiece(rank-1, file)))
            ...
            if (isAttackableNoVoid(piece, getPiece(rank, file+1)))
            {
            ...
            if (rankInBounds(rank+1) && isAttackableNoVoid(piece, getPiece(rank+1, file+1)))
            {
                moves.Add(new Move{rankFrom = rank, fileFrom = file, rankTo = (byte)(rank+1), fileTo = fileOverload(file+1)});
            }
            if (rankInBounds(rank+1) && isAttackableNoVoid(piece, getPiece(rank+1, file-1)))
